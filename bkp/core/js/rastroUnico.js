              ...
              ${detalhes
                .map((detalhe) => {
                  ...
                  if (url) {
                    ...
                    return `<p class="text text-head">${detalhe}</p>`; // Retorna o detalhe original se não houver URL
                  }
                })
                .join("")}
...
const divArrow = (arrow, icone) => {
  return `
        <div class="${arrow}">
            <div class="circle">
            <img class="circle-img" src="../static/rastreamento-internet/imgs/${icone}">            
            </div>
        </div>
    `;
};
...
const caminhoNacional = {
  normal: function (
    { unidade = "", unidade: { tipo = "" }, detalhe = "", comentario = "" },
    percorridaCarteiro
  ) {
  ...
  transito: function (
    { unidade = "", unidade: { tipo = "" }, unidadeDestino = "", detalhe = "" },
    pc
  ) {
    const cidadeUf = getCidadeUf(unidade.endereco);
